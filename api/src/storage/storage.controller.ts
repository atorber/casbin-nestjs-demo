...
export class StorageController {
  ...
  @ApiResponse({ status: 201, description: '存储路径创建成功', type: StoragePath })
  ...
  async createStoragePath(
    ...
    return this.storageService.createStoragePath(createStoragePathDto, userId);
  }

  @Get('paths')
  @RequirePermissions('storage', 'read')
  @ApiOperation({ summary: '获取所有存储路径（仅管理员）' })
  @ApiResponse({ status: 200, description: '返回所有存储路径', type: [StoragePath] })
  @ApiResponse({ status: 403, description: '禁止访问 - 仅限管理员' })
  ...
  @ApiResponse({ status: 200, description: '返回存储路径信息', type: StoragePath })
  @ApiResponse({ status: 403, description: '禁止访问 - 仅限管理员' })
  @ApiResponse({ status: 404, description: '存储路径不存在' })
  async getStoragePathById(@Param('id', ParseIntPipe) id: number): Promise<StoragePath> {
  ...
  @ApiResponse({ status: 403, description: '禁止访问 - 仅限管理员' })
  @ApiResponse({ status: 404, description: '存储路径不存在' })
  async deleteStoragePath(@Param('id', ParseIntPipe) id: number): Promise<void> {
  ...
  @ApiResponse({ status: 201, description: '权限授予成功', type: StoragePermissionEntity })
  @ApiResponse({ status: 403, description: '禁止访问 - 仅限管理员' })
  ...
  @ApiResponse({ status: 200, description: '返回所有存储权限', type: [StoragePermissionEntity] })
  @ApiResponse({ status: 403, description: '禁止访问 - 仅限管理员' })
  ...
  @ApiResponse({ status: 200, description: '返回当前用户的存储权限', type: [StoragePermissionEntity] })
  @ApiResponse({ status: 401, description: '未授权' })
  async getMyStoragePermissions(@Request() req): Promise<StoragePermissionEntity[]> {
    const userId = req.user?.id || req.user?.userId;
    if (!userId) {
      throw new Error('无法获取用户ID');
    }
    return this.storageService.getUserStoragePermissions(userId);
  }
}
